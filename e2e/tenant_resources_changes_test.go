...
var _ = Describe("changing Tenant managed Kubernetes resources", func() {
	...
	It("should reapply the original Limit Range resources upon third party change", func() {
		for _, ns := range nsl {
			for i, s := range tnt.Spec.LimitRanges {
				...
				Eventually(func() corev1.LimitRangeSpec {
					...
					return lr.Spec
				}, 10*time.Second, time.Second).Should(Equal(s))
			}
		}
	})
	It("should reapply the original Network Policy resources upon third party change", func() {
		for _, ns := range nsl {
	...
	It("should contains all Resource Quota resources", func() {
		for _, ns := range nsl {
